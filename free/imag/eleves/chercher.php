...
if ( ($condition != "")||($nom_form !="")||($prenom_form !="")||($adresse_form !="")||($note_form !="") ){//un des champs est NON vide : la recherche est lancee
	...
	if ($condition == "") {	// si oui ca veut dire que c'est la 1ere fois, cad  y a pas d'ordre d�finit
		...
		$condition = "";									// initialisation � la chaine vide
		...
		while ($i < ($nb_champ_non_vides-1) ) {				// -1 pour s'arreter � l'avnt dernier
	...
	elseif ($ordre != "") {			//cad qu'un ordre est d�finit (on a cliqu� sur une des fl�ches)
		...
		// Maintenant que la requete a faire est d�termin�e , on a plus qu'� afficher les r�sultats

	$resultat = requete( $requete );	//questionnement de la base de donn�es
	$nb_res = mysql_num_rows($resultat);
	if ($nb_res >0) {
		echo "<CENTER>Il y a <B>$nb_res</B> �l�ve".($nb_res >1 ?"s":"")." dont le profil correspond � la recherche</CENTER>";
		?>
		<CENTER>
			<H4><FONT COLOR="4242FF">R�sultat de la recherche :</FONT></H4>
		</CENTER>
			...
			<TR bgcolor="#0000BB" align="center">
				...
				<TD>
					<FONT COLOR="white">
						...
						<B>Nom</B>
						<A HREF="index.php3?to=chercher&ordre=nom&sens= DESC&condition=<?echo $condition;?>"><IMG SRC="fleche_bas_icone.gif" WIDTH="12" HEIGHT="14" BORDER=0 ALT="TRIER PAR ORDRE DECROISSANT"></A>
					</FONT>
				</TD>
				<TD>
					<FONT COLOR="white">
		...
		while( $row = mysql_fetch_array( $resultat ) ) {
			?>
			<TR bgcolor="#CCCCFF" align="center">	<!-- une ligne de tableau par resultat trouv� -->
		...
		echo "<CENTER><B>Aucun</B> profil ne correspond � votre recherche</CENTER>"; // zero resultats
	}
	...
	echo "<CENTER>Remplissez au moins un champ de ce formulaire et validez.<BR><B>Consignes</B> : un seul champ suffit, <FONT COLOR='red'>_</FONT> remplasse UN caractere et  &nbsp;<FONT COLOR='red'>%</FONT> en remplace plusieurs<BR>Cliquez sur le petit livre pour plus d'information.<BR><A HREF=\"#\" onClick=\"script_aide('aide_chercher.html');\"><IMG SRC=\"aide_icone.gif\" WIDTH=\"21\" HEIGHT=\"21\" BORDER=\"0\" ALT=\"AIDE\"></A></CENTER>";	//message d'accueil
}
?>
